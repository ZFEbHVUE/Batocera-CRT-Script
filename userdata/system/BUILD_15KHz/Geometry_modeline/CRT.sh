...
escape=$(grep -c "Aborted!" /userdata/roms/ports/temp_crt.txt)
if [[ "$escape" -ge 1 ]]; then
...
sed -i '1,2d' /userdata/roms/ports/temp_crt.txt
sed -i 's/.*monitor         .*/monitor                   custom/' /userdata/system/configs/mame/mame.ini
sed -i '/Final geometry/d' /userdata/roms/ports/temp_crt.txt
CRT_0="$(cat /userdata/roms/ports/temp_crt.txt)"
sed -i "s/^crt_range0.*/$CRT_0/" /userdata/system/configs/mame/mame.ini
sed -i 's/.*monitor         .*/	monitor                   custom/' /etc/switchres.ini
sed -i '/Final geometry/d' /userdata/roms/ports/temp_crt.txt
sed -i 's/crt_range0/        crt_range0/g' /userdata/roms/ports/temp_crt.txt
...
do
	...
	switchres $RESOLUTION_TOT -f $FORCED_RESOLUTION -m custom -c | tee /userdata/roms/ports/temp_mode.txt >> /userdata/system/logs/custom_crt_monitor.log
	sed -i '/Calculating best video mode/d' /userdata/roms/ports/temp_mode.txt
	sed -i 's/^.*"//' /userdata/roms/ports/temp_mode.txt
	if [[ "$TYPE_OF_CARD" == "AMD_INTEL_NVIDIA_NOUV" ]]; then
		sed -i 's|^|xrandr -display :0.0 --newmode "'"$RESOLUTION"'" |' /userdata/roms/ports/temp_mode.txt
	else
		sed -i 's|^|"'"$RESOLUTION"'" |' /userdata/roms/ports/temp_mode.txt
	fi
	TERM="$RESOLUTION"
	MODE_CONTENT="$(cat /userdata/roms/ports/temp_mode.txt)"
	if [[ "$TYPE_OF_CARD" == "AMD_INTEL_NVIDIA_NOUV" ]]; then
	...
	sed -i '1d' /userdata/roms/ports/temp_mode.txt
done
...
mv /userdata/roms/ports/temp_crt.txt /userdata/roms/ports/crt_range_0_mod.log
rm /userdata/roms/ports/temp_mode.txt
