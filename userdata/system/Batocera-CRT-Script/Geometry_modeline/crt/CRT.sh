...
rollback(){
	...
	3)#switchres.ini, mame.ini and (custom-es-config or 99-nvidia)
			...
			cp /boot/EFI/syslinux.cfg.crt 		/boot/EFI/syslinux.cfg
			cp /boot/EFI/BOOT/syslinux.cfg.crt 	/boot/EFI/BOOT/syslinux.cfg
			cp /boot/boot/syslinux.cfg.crt 		/boot/boot/syslinux.cfg
			cp /boot/boot/syslinux/syslinux.cfg.crt /boot/boot/syslinux/syslinux.cfg
...
if [ -f /userdata/system/99-nvidia.conf ]; then
	if  [ ! -f "/userdata/system/99-nvidia.conf.bak" ]; then
		...
		cp /boot/EFI/syslinux.cfg 		/boot/EFI/syslinux.cfg.crt
		cp /boot/EFI/BOOT/syslinux.cfg 		/boot/EFI/BOOT/syslinux.cfg.crt
		cp /boot/boot/syslinux.cfg 		/boot/boot/syslinux.cfg.crt
		cp /boot/boot/syslinux/syslinux.cfg 	/boot/boot/syslinux/syslinux.cfg.crt

		cp /boot/EFI/syslinux.cfg.bak 	 	/boot/EFI/syslinux.cfg
		cp /boot/EFI/BOOT/syslinux.cfg.bak 	/boot/EFI/BOOT/syslinux.cfg
		cp /boot/boot/syslinux.cfg.bak 	 	/boot/boot/syslinux.cfg
		cp /boot/boot/syslinux/syslinux.cfg.bak /boot/boot/syslinux/syslinux.cfg
...
if [[ ! -f /userdata/system/99-nvidia.conf ]]; then
	...
	if [[ "$MONITOR" == "arcade_25" ]]; then
        	...
        	RESOLUTIONS=(	"3600x480 60" "1920x240 60" "1920x256 50" "1920x480 60" "2560x256 50" "2560x448 60" "1280x480 60" \
                        	"1024x576 50" "768x576 50"  "960x480 60" "960x540 50" "854x480 60" "864x486 50"  "800x576 50" "720x480 60" \
                        	"640x480 60" "640x240 60" "576x448 60" "512x480 60" "512x288 50" "480x270 50" "480x240 60" "456x256 60" \
                        	"432x240 60" "428x240 60" "427x240 60" "426x240 60" "416x240 60" "400x240 60" "400x224 50" "384x240 60" \
                        	"360x240 60" "320x256 50" "320x240 60" "256x240 60")
	elif [[ "$MONITOR" == "generic_15" ]]; then
...
####################################################################################
#                                                                                  #
#                           creation of a custom.bin EDID                          #
#                                                                                  #
####################################################################################
if grep -q "NOUVEAU" 		/userdata/system/logs/TYPE_OF_CARD_DRIVERS.info ; then
	switchres 1280 480 60 -e
elif grep -q "Nvidia_Drivers" 	/userdata/system/logs/TYPE_OF_CARD_DRIVERS.info ; then
	switchres 1280 480 60 -e
elif grep -q "INTEL" 		/userdata/system/logs/TYPE_OF_CARD_DRIVERS.info ; then
	switchres 1280 480 60 -e
else
	switchres 640 480 60 -e
fi
cp /userdata/custom.bin /lib/firmware/edid/
rm /userdata/custom.bin
sed -i 's@edid/[^ ]*@edid/custom.bin@' /boot/EFI/syslinux.cfg
#############################################################################
