...
backup_video_settings() {
    ...
    if [ -f "/userdata/system/batocera.conf" ]; then
        ...
        # Save es.resolution separately — it may have higher precision than
        # global.videomode (e.g. 769x576.50.00060 vs 769x576.50.00).
        # ES matches es.resolution against batocera-resolution listModes;
        # a truncated value causes ES to show "Auto" instead of the boot mode.
        # Only write if the UI hasn't already set it (UI writes during run_mode_switch_ui).
        if [ ! -s "${backup_dir}/es_resolution.txt" ]; then
            grep "^es\.resolution=" /userdata/system/batocera.conf > "${backup_dir}/es_resolution.txt" 2>/dev/null || true
        fi
        # Note: video_mode.txt guard (! -s) is kept because the UI writes it
        # first in run_mode_switch_ui — backup should not overwrite the UI's value.
...
restore_video_settings() {
    ...
    if [ -f "${backup_dir}/video_mode.txt" ]; then
        ...
        if [ -n "$video_mode_line" ]; then
            ...
            # Determine the correct es.resolution value.
            # Prefer the separately backed-up es.resolution (full precision,
            # e.g. 769x576.50.00060) over deriving from global.videomode
            # (which may be truncated, e.g. 769x576.50.00).
            local es_res_value="$mode_id"
            if [ -f "${backup_dir}/es_resolution.txt" ]; then
                local backed_up_es_res
                backed_up_es_res=$(head -1 "${backup_dir}/es_resolution.txt" | cut -d'=' -f2)
                if [ -n "$backed_up_es_res" ]; then
                    es_res_value="$backed_up_es_res"
                    echo "[$(date +"%H:%M:%S")]: Using backed-up es.resolution=$es_res_value (full precision)" >> "$LOG_FILE"
                fi
            fi
            
            # CRITICAL: Write es.resolution to BOTH config files
            # ES checks both batocera.conf AND batocera-boot.conf
            if [ -n "$es_res_value" ]; then
                # Write to batocera.conf
                if grep -q "^es.resolution=" /userdata/system/batocera.conf 2>/dev/null; then
                    sed -i "s|^es.resolution=.*|es.resolution=$es_res_value|" /userdata/system/batocera.conf
                else
                    echo "es.resolution=$es_res_value" >> /userdata/system/batocera.conf
                fi
                
                # Write to batocera-boot.conf (ensure /boot is writable)
                mount -o remount,rw /boot 2>/dev/null || true
                if grep -q "^es.resolution=" /boot/batocera-boot.conf 2>/dev/null; then
                    sed -i "s|^es.resolution=.*|es.resolution=$es_res_value|" /boot/batocera-boot.conf
                else
                    echo "es.resolution=$es_res_value" >> /boot/batocera-boot.conf
                fi
                echo "[$(date +"%H:%M:%S")]: Set es.resolution=$es_res_value in both config files" >> "$LOG_FILE"
            fi
        fi
    else
...
backup_mode_files() {
    ...
    if is_dualboot_system; then
        ...
        # For CRT Mode: Find the master syslinux.cfg that contains CRT kernel
        # parameters (drm.edid_firmware= / video=) and use it as the single
        # source of truth for all locations.
        # /boot/EFI/syslinux.cfg is the EFI boot entry actually read at boot;
        # /boot/boot/syslinux.cfg and variants must be identical copies of it.
        if [ "$mode" = "crt" ]; then
            local CRT_SYSLINUX_MASTER=""
            for candidate in "/boot/EFI/syslinux.cfg" \
                             "/boot/EFI/batocera/syslinux.cfg" \
                             "/boot/EFI/BOOT/syslinux.cfg" \
                             "/boot/boot/syslinux.cfg" \
                             "/boot/boot/syslinux/syslinux.cfg"; do
                if [ -f "$candidate" ] && grep -q "drm.edid_firmware\|video=.*:e" "$candidate" 2>/dev/null; then
                    CRT_SYSLINUX_MASTER="$candidate"
                    echo "[$(date +"%H:%M:%S")]: CRT Mode: Found master syslinux.cfg with EDID params: $candidate" >> "$LOG_FILE"
                    break
                fi
            done

            if [ -n "$CRT_SYSLINUX_MASTER" ]; then
                # Save master as backup — used as fallback in restore
                mkdir -p "${backup_dir}/boot_configs/boot"
                mkdir -p "${backup_dir}/boot_configs/EFI"
                cp -a "$CRT_SYSLINUX_MASTER" "${backup_dir}/boot_configs/boot/syslinux.cfg" 2>/dev/null || true
                cp -a "$CRT_SYSLINUX_MASTER" "${backup_dir}/boot_configs/EFI/syslinux.cfg" 2>/dev/null || true
                echo "[$(date +"%H:%M:%S")]: CRT Mode: Backed up master syslinux.cfg from $CRT_SYSLINUX_MASTER" >> "$LOG_FILE"
            else
                echo "[$(date +"%H:%M:%S")]: WARNING: No syslinux.cfg with EDID parameters found — falling back to /boot/boot/syslinux.cfg" >> "$LOG_FILE"
...
restore_mode_files() {
    ...
    if [ "$mode" = "hd" ]; then
        ...
        if is_dualboot_system; then
            ...
            # Save current CRT syslinux.cfg to CRT backup BEFORE overwriting it.
            # /boot/EFI/syslinux.cfg currently has the correct EDID params (we are
            # in CRT mode). Once HD restore propagates its vanilla file everywhere,
            # this live copy will be gone — so we preserve it now for the next
            # CRT restore to use.
            if [ -f "/boot/EFI/syslinux.cfg" ] && grep -q "drm.edid_firmware\|video=.*:e" "/boot/EFI/syslinux.cfg" 2>/dev/null; then
                mkdir -p "${MODE_BACKUP_DIR}/crt_mode/boot_configs/boot"
                mkdir -p "${MODE_BACKUP_DIR}/crt_mode/boot_configs/EFI"
                cp -a "/boot/EFI/syslinux.cfg" "${MODE_BACKUP_DIR}/crt_mode/boot_configs/boot/syslinux.cfg" 2>/dev/null || true
                cp -a "/boot/EFI/syslinux.cfg" "${MODE_BACKUP_DIR}/crt_mode/boot_configs/EFI/syslinux.cfg" 2>/dev/null || true
                echo "[$(date +"%H:%M:%S")]: Pre-saved CRT syslinux.cfg before HD overwrite" >> "$LOG_FILE"
            fi

            # Determine HD master source: factory backup (first install) or HD mode backup
            local hd_master_src=""
            if [ "$use_crt_backup" = true ]; then
                # First install: use factory syslinux.cfg saved by CRT Script installer
                for candidate in "${CRT_BACKUP_DIR}/boot/EFI/syslinux.cfg" \
                                 "${CRT_BACKUP_DIR}/boot/boot/syslinux.cfg"; do
                    if [ -f "$candidate" ]; then
                        hd_master_src="$candidate"
                        echo "[$(date +"%H:%M:%S")]: HD Mode: Using factory syslinux.cfg from $candidate" >> "$LOG_FILE"
                        break
                    fi
                done
            else
                # Subsequent switches: use HD mode backup
                for candidate in "${backup_dir}/boot_configs/EFI/syslinux.cfg" \
                                 "${backup_dir}/boot_configs/boot/syslinux.cfg"; do
                    if [ -f "$candidate" ]; then
                        hd_master_src="$candidate"
                        echo "[$(date +"%H:%M:%S")]: HD Mode: Using syslinux.cfg from HD backup: $candidate" >> "$LOG_FILE"
                        break
                    fi
                done
            fi

            # Propagate HD master syslinux.cfg to ALL locations (identical copies)
            if [ -n "$hd_master_src" ]; then
                for dest in "/boot/EFI/syslinux.cfg" \
                            "/boot/EFI/batocera/syslinux.cfg" \
                            "/boot/EFI/BOOT/syslinux.cfg" \
                            "/boot/boot/syslinux.cfg" \
                            "/boot/boot/syslinux/syslinux.cfg"; do
                    ...
                    [ -d "$dest_dir" ] || continue
                    cp -a "$hd_master_src" "$dest" 2>/dev/null || true
                    chmod 644 "$dest" 2>/dev/null || true
                    echo "[$(date +"%H:%M:%S")]: Restored HD syslinux.cfg to $dest" >> "$LOG_FILE"
                done
            else
                echo "[$(date +"%H:%M:%S")]: WARNING: No HD syslinux.cfg source found — boot config unchanged" >> "$LOG_FILE"
            fi
        fi
...
BOOTCUSTOM_EOF
        ...
        if [ -f "$_es_cfg" ] && grep -q 'name="HideWindow"' "$_es_cfg" 2>/dev/null; then
            ...
            echo "[$(date +"%H:%M:%S")]: Removed HideWindow from es_settings.cfg (HD mode)" >> "$LOG_FILE"
        fi
        ...
        if ! is_dualboot_system; then
            if mount -o remount,rw /boot 2>/dev/null; then
                rm -rf "/boot/crt_theme_assets" 2>/dev/null || true
                # Note: do NOT remount read-only here — step 4 still needs to write syslinux.cfg
        ...
        if is_dualboot_system; then
            ...
            # Single-boot: propagate CRT syslinux.cfg to ALL locations.
            # Priority 1: live /boot/EFI/syslinux.cfg — guaranteed correct since
            #             it's the file that actually booted CRT mode.
            # Priority 2: backup file — fallback if live file has no EDID params.
            local master_cfg=""
            if [ -f "/boot/EFI/syslinux.cfg" ] && grep -q "drm.edid_firmware\|video=.*:e" "/boot/EFI/syslinux.cfg" 2>/dev/null; then
                master_cfg="/boot/EFI/syslinux.cfg"
                echo "[$(date +"%H:%M:%S")]: CRT syslinux master: using live /boot/EFI/syslinux.cfg" >> "$LOG_FILE"
            elif [ -f "${backup_dir}/boot_configs/boot/syslinux.cfg" ]; then
                master_cfg="${backup_dir}/boot_configs/boot/syslinux.cfg"
                echo "[$(date +"%H:%M:%S")]: CRT syslinux master: using backup (live file had no EDID params)" >> "$LOG_FILE"
            fi
            ...
            if [ -n "$master_cfg" ]; then
                for dest in "/boot/EFI/syslinux.cfg" \
                    ...
                    dest_dir=$(dirname "$dest")
                    [ -d "$dest_dir" ] || continue
                    # Skip source file to avoid overwriting it with itself
                    [ "$dest" = "$master_cfg" ] && continue
                    cp -a "$master_cfg" "$dest" 2>/dev/null || true
                    chmod 644 "$dest" 2>/dev/null || true
                    echo "[$(date +"%H:%M:%S")]: Restored CRT syslinux.cfg to $dest" >> "$LOG_FILE"
                done
            else
                echo "[$(date +"%H:%M:%S")]: WARNING: No CRT syslinux.cfg with EDID params found — EDID params may be missing" >> "$LOG_FILE"
            fi
        fi
    ...
    # set_crt_gamelist_visibility <gamelist_path> <mode>
    # HD mode:  adds <hidden>true</hidden> to every entry except mode_switcher.sh
    # CRT mode: removes all <hidden> tags so every entry is visible
    set_crt_gamelist_visibility() {
        local gamelist="$1"
        local vis_mode="$2"
        local tmp="${gamelist}.tmp"

        if [ ! -f "$gamelist" ]; then
            echo "[$(date +"%H:%M:%S")]: WARNING: gamelist.xml not found at $gamelist, skipping visibility update" >> "$LOG_FILE"
            return 1
        fi

        if [ "$vis_mode" = "hd" ]; then
            # Add <hidden>true</hidden> after every <path> that is NOT mode_switcher.sh.
            # Remove any pre-existing <hidden> lines first to avoid duplicates.
            awk '
                /<path>\.\/mode_switcher\.sh<\/path>/ { in_switcher = 1 }
                /<\/game>/                            { in_switcher = 0 }
                /<hidden>/                            { next }
                /<path>/ && !in_switcher {
                    print
                    print "\t\t<hidden>true</hidden>"
                    next
                }
                { print }
            ' "$gamelist" > "$tmp" && mv "$tmp" "$gamelist" 2>/dev/null || true
            echo "[$(date +"%H:%M:%S")]: Updated gamelist.xml: mode_switcher.sh visible, CRT tools hidden" >> "$LOG_FILE"
        else
            # Remove all <hidden> tags — every entry is shown in CRT mode
            sed '/<hidden>/d' "$gamelist" > "$tmp" && mv "$tmp" "$gamelist" 2>/dev/null || true
            echo "[$(date +"%H:%M:%S")]: Updated gamelist.xml: all CRT tools visible" >> "$LOG_FILE"
        fi
        chmod 644 "$gamelist" 2>/dev/null || true
    }
    ...
    # Step 2: Copy all CRT Tools (both modes always keep all files present)
    # gamelist.xml visibility controls what is shown in EmulationStation per mode
    local src_dir="/userdata/system/Batocera-CRT-Script/Geometry_modeline/crt"
    if [ -d "$src_dir" ]; then
        cp -a "${src_dir}/." "$CRT_ROMS/crt/" 2>/dev/null || true
    fi
    ...
    # Restore or recreate GunCon2_Calibration.sh (always present, hidden in HD via gamelist)
    if [ -f "${backup_dir}/userdata_configs/GunCon2_Calibration.sh" ]; then
        ...
        echo "[$(date +"%H:%M:%S")]: Restored GunCon2_Calibration.sh from backup" >> "$LOG_FILE"
    else
    ...
    # Update gamelist.xml visibility: HD = only mode_switcher visible; CRT = all visible
    ...
    # Step 5: Set permissions (all modes — files are always present)
    ...
    # Step 9: Copy overlays_overrides.sh (all modes — hidden in HD via gamelist)
