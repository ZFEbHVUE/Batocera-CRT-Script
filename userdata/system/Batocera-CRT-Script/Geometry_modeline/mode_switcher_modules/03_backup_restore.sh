...
BOOTCUSTOM_EOF
    ...
    # Step 2: Refresh scripts and assets from source — files are NEVER deleted so
    # NAS-mounted roms survive mode switches. Visibility is controlled by gamelist.xml.
    local src_dir="/userdata/system/Batocera-CRT-Script/Geometry_modeline/crt"
    if [ -d "$src_dir" ]; then
        cp -a "${src_dir}/." /userdata/roms/crt/ 2>/dev/null || true
    fi
    # CRT system icons (needed for ES system artwork in both modes)
    if [ -f "/userdata/system/Batocera-CRT-Script/Geometry_modeline/CRT.svg" ]; then
    ...
    if [ "$mode" = "hd" ]; then
        echo "[$(date +"%H:%M:%S")]: HD Mode: Hiding CRT-only tools in gamelist.xml" >> "$LOG_FILE"
    else
        # CRT Mode only: restore personalized GunCon2 calibration script
        ...
        echo "[$(date +"%H:%M:%S")]: CRT Mode: Showing all CRT tools in gamelist.xml" >> "$LOG_FILE"
    fi

    # Update gamelist.xml visibility for the target mode
    set_crt_gamelist_visibility /userdata/roms/crt/gamelist.xml "$mode"
