	...
	if [ -z "$choice_MAME_monitor" ] ; then 
		...
		if [ "$monitor_MAME_CHOICE" == "YES" ]; then
			...
			if [ "$TYPE_OF_CARD" == "INTEL" ] ||  [ "$TYPE_OF_CARD" == "NVIDIA" ]; then 
				declare -a type_of_monitor_mame=( "arcade_15" "arcade_15_SR480" "arcade_15_SR240" "arcade_15_25_31"  \
								  "arcade_15ex"  "generic_15" "generic_15_SR480" "generic_15_SR240" "ntsc"  "pal" )
			else
				declare -a type_of_monitor_mame=( "arcade_15"  "arcade_15_25_31"  \
								 "arcade_15ex"  "generic_15" "ntsc" "pal" )
			fi
			for var in "${!type_of_monitor_mame[@]}" ; do echo "			$((var+1)) : ${type_of_monitor_mame[$var]}"; done
			echo ""
			echo "#######################################################################"
			...
			monitor_firmware_MAME=${type_of_monitor_mame[$monitor_choice_MAME-1]}
...
if [[ -z "$choice_DT_SR" || $choice_DT_SR = "1" ]] ; then 
...
for i in $var_choix; do echo -e "                    your choice is : ${Encoder_inputs[$i-1]}" ; touch /userdata/system/configs/xarcade2jstick/${Encoder_inputs[$((i-1))]};done
else 
...
#default calibration values for Guncon2 
#	Guncon2_x=320
#	Guncon2_y=240
#	Guncon2_f=60
#	Guncon2_res="320x240"
#	Guncon2_8MHz=385
	...
	sed -e "s/\[guncon2_x\]/$Guncon2_x/g" -e "s/\[guncon2_y\]/$Guncon2_y/g" -e "s/\[guncon2_f\]/$Guncon2_f/g" -e "s/\[guncon2_res\]/$Guncon2_res/g" \
