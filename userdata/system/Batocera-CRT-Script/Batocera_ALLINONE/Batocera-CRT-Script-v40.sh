...
echo "##                                                                   ##"
echo "##	    WE SEND CODOLENCES TO HIS FAMILY AND FIRENDS             ##"
echo "##                                                                   ##"
...
if [ "$Drivers_Nvidia_CHOICE" == "Nvidia_Drivers" ]; then
	...
	if [ "$Drivers_Nvidia_CHOICE" == "NOUVEAU" ]&&([ "$Version_of_batocera" == "v39" ]||[ "$Version_of_batocera" == "v40" ]); then
		sed -e 's/.*nvidia-driver=.*/nvidia-driver=false/' -e 's/.*amdgpu=.*/#amdgpu=true/' 	 	/boot/batocera-boot.conf > /boot/batocera-boot.conf.tmp
	else
		if [ "$TYPE_OF_CARD" == "AMD/ATI" ]&&([ "$Version_of_batocera" == "v39" ]||[ "$Version_of_batocera" == "v40" ]); then
