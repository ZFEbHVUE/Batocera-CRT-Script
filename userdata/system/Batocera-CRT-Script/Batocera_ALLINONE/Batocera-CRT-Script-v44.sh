...
if [ "$Version_of_batocera" != "v43" ]&&[ "$Version_of_batocera" != "v44" ]; then
  box_hash
  box_center "${RED}WRONG BATOCERA VERSION${NOCOLOR}"
  box_center "This script requires Batocera v43 or v44."
...
########################################################################################
# Target Batocera major version
if ([ "$Version_of_batocera" == "v43" ]); then
{
   TARGET_VER="43"
}
elif ([ "$Version_of_batocera" == "v44" ]); then
{
   TARGET_VER="44"
}
else
...
# Channel bases
BETA_BASE="https://mirrors.o2switch.fr/batocera/x86_64/butterfly/last"
STABLE_BASE="https://mirrors.o2switch.fr/batocera/x86_64/stable/last"
# Helper: return the image filename for $TARGET_VER in a given base, or empty
find_image() {
    local base="$1"
    curl -s "$base/" \
        | grep -oE "batocera-x86_64-${TARGET_VER}(\.[0-9]+)?-[0-9]+\.img\.gz" \
        | sort -u | tail -1
}

# Prefer beta (butterfly) if it still carries version 44, else use stable
IMG_NAME=$(find_image "$BETA_BASE")
if [ -n "$IMG_NAME" ]; then
    BATO_BASE="$BETA_BASE"
    echo "Found v$TARGET_VER on BETA (butterfly): $IMG_NAME"
else
    IMG_NAME=$(find_image "$STABLE_BASE")
    BATO_BASE="$STABLE_BASE"
    echo "v$TARGET_VER no longer on butterfly → using STABLE: $IMG_NAME"
fi

if [ -z "$IMG_NAME" ]; then
    echo "ERROR: Batocera v$TARGET_VER not found on either channel" >&2
    exit 1
fi

X11_IMAGE_URL="$BATO_BASE/$IMG_NAME"
X11_MD5_URL="$BATO_BASE/${IMG_NAME}.md5"
...
update_boot_config() {
  ...
  # Batocera v43 or v43 uses syslinux for both EFI and Legacy BIOS boot.
...
# Resolve the X authority cookie so xrandr works from SSH / non-ES contexts.
# Batocera starts X with `-auth /<something>/.serverauth.<pid>`; without pointing
# XAUTHORITY at that file, xrandr fails with "Authorization required".
ensure_x_authority() {
  ...
  local xauth=""
  xauth="$(ps -eo args 2>/dev/null | grep -m1 -oE '/[^ ]*\.serverauth\.[0-9]+' || true)"
  if [ -z "$xauth" ] || [ ! -f "$xauth" ]; then
    xauth="$(ls -t /.serverauth.* /root/.serverauth.* /var/run/.serverauth.* 2>/dev/null | head -1)"
  fi
  if [ -z "$xauth" ] || [ ! -f "$xauth" ]; then
    for cand in /root/.Xauthority "$HOME/.Xauthority" /tmp/.Xauthority; do
      [ -f "$cand" ] && { xauth="$cand"; break; }
    done
  fi
  if [ -n "$xauth" ] && [ -f "$xauth" ]; then
    export XAUTHORITY="$xauth"
    echo "ensure_x_authority: using XAUTHORITY=$xauth" >> "${LOG_FILE:-/tmp/crt_script.log}" 2>/dev/null
  fi
  if ! xrandr --query >/dev/null 2>&1; then
...
printf "%s\n" "$border"
empty_line
center "BATOCERA CRT SCRIPT — 15 kHz / 25 kHz / 31 kHz for Batocera v43/v44"
empty_line
...
center "• net_terminal_gene — HD/CRT Mode Switcher, Wayland/X11 dual-boot support"
empty_line
...
if [ "$Drivers_Nvidia_CHOICE" == "Nvidia_Drivers" ]; then
	...
	if [ "$Drivers_Nvidia_CHOICE" == "NOUVEAU" ]&&([ "$Version_of_batocera" == "v43" ]||[ "$Version_of_batocera" == "v44" ]); then
		sed -e 's/.*nvidia-driver=.*/nvidia-driver=false/' -e 's/.*amdgpu=.*/#amdgpu=true/' 	 	/boot/batocera-boot.conf > /boot/batocera-boot.conf.tmp
	else
		if [ "$TYPE_OF_CARD" == "AMD/ATI" ]&&([ "$Version_of_batocera" == "v43" ]||[ "$Version_of_batocera" == "v44" ]); then
...
case $Version_of_batocera in
    ...
    v43)
		cp /userdata/system/Batocera-CRT-Script/UsrBin_configs/Configgen/emulatorlauncher.py_v43_ZFEbHVUE	"${new_path1}emulatorlauncher.py"
		chmod 755  "${new_path1}emulatorlauncher.py"
		cp /userdata/system/Batocera-CRT-Script/UsrBin_configs/Configgen/VideoMode.py-v43_ZFEbHVUE		"${new_path2}videoMode.py"
		chmod 755 "${new_path2}videoMode.py"

		if [ "$Drivers_Nvidia_CHOICE" == "Nvidia_Drivers" ]; then
		   cp /userdata/system/Batocera-CRT-Script/UsrBin_configs/batocera-resolution-v43_Nvidia_driver_MYZAR_ZFEbHVUE-MultiScreen 	/usr/bin/batocera-resolution
		else
		   cp /userdata/system/Batocera-CRT-Script/UsrBin_configs/batocera-resolution-v43_MYZAR_ZFEbHVUE-MultiScreen	/usr/bin/batocera-resolution
		fi
		cp /userdata/system/Batocera-CRT-Script/UsrBin_configs/emulationstation-standalone-v43_ZFEbHVUE-MultiScreen	/usr/bin/emulationstation-standalone
	...
	;;
    v44)
		cp /userdata/system/Batocera-CRT-Script/UsrBin_configs/Configgen/emulatorlauncher.py_v44_ZFEbHVUE	"${new_path1}emulatorlauncher.py"
		chmod 755  "${new_path1}emulatorlauncher.py"
		cp /userdata/system/Batocera-CRT-Script/UsrBin_configs/Configgen/VideoMode.py-v44_ZFEbHVUE		"${new_path2}videoMode.py"
		chmod 755 "${new_path2}videoMode.py"
		
		if [ "$Drivers_Nvidia_CHOICE" == "Nvidia_Drivers" ]; then
		   cp /userdata/system/Batocera-CRT-Script/UsrBin_configs/batocera-resolution-v44_Nvidia_driver_MYZAR_ZFEbHVUE-MultiScreen 	/usr/bin/batocera-resolution
		else
		   cp /userdata/system/Batocera-CRT-Script/UsrBin_configs/batocera-resolution-v44_MYZAR_ZFEbHVUE-MultiScreen	/usr/bin/batocera-resolution

		fi
		cp /userdata/system/Batocera-CRT-Script/UsrBin_configs/emulationstation-standalone-v44_ZFEbHVUE-MultiScreen	/usr/bin/emulationstation-standalone
...
#######################################################################################
## Bootstrap videomode/videooutput — capture HD state and derive CRT Boot_ mode
#######################################################################################
MODE_BACKUP_BASE="/userdata/Batocera-CRT-Script-Backup/mode_backups"

# Step 1: Capture current HD values (batocera.conf still reflects HD state at this point)
...
# Step 2: Write HD backup files (mode switcher reads these on CRT->HD switch)
mkdir -p "${MODE_BACKUP_BASE}/hd_mode/video_settings"
echo "global.videomode=${_hd_videomode:-default}" \
    > "${MODE_BACKUP_BASE}/hd_mode/video_settings/video_mode.txt"
echo "${_hd_videooutput}" \
    > "${MODE_BACKUP_BASE}/hd_mode/video_settings/video_output.txt"

# Step 3: Derive CRT Boot_ mode from installed videomodes.conf
...
if [ -n "$_crt_boot_mode" ]; then
    echo "global.videomode=$_crt_boot_mode" \
        > "${MODE_BACKUP_BASE}/crt_mode/video_settings/video_mode.txt"
fi
if [ -n "$video_output_xrandr" ]; then
...
# Step 6: Fix es.resolution to use the correct videomodes.conf key.
# The installer writes es.resolution with a .00000 suffix which does not match any
# key in batocera-resolution listModes, causing ES to show "AUTO" in Video Mode.
# Replace with the exact key derived from videomodes.conf (e.g. 641x480.60.00059).
if [ -n "$_crt_boot_mode" ]; then
