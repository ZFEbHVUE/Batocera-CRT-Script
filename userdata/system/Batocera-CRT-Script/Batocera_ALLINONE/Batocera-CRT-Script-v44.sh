...
if [ "$Version_of_batocera" != "v44" ]; then
  box_hash
  box_center "${RED}WRONG BATOCERA VERSION${NOCOLOR}"
  box_center "This script requires Batocera v44."
...
# You can remove this after you’re sure nothing reads "version".
version="${VERSION_BATOCERA_NUM}"
...
update_boot_config() {
  ...
  # Batocera v44 uses syslinux for both EFI and Legacy BIOS boot.
...
# Batocera starts X with `-auth /<path>/.serverauth.<pid>`; without pointing
# XAUTHORITY at that file, xrandr fails with "Authorization required".
ensure_x_authority() {
  export DISPLAY="${DISPLAY:-:0}"

  # Already able to talk to X? nothing to do.
  if xrandr --query >/dev/null 2>&1; then
    return 0
  fi

  # a) exact -auth path from the running X server command line
  local xauth=""
  xauth="$(ps -eo args 2>/dev/null | grep -m1 -oE '/[^ ]*\.serverauth\.[0-9]+' || true)"

  # b) fallback: newest .serverauth.* in common locations
  if [ -z "$xauth" ] || [ ! -f "$xauth" ]; then
    xauth="$(ls -t /.serverauth.* /root/.serverauth.* /var/run/.serverauth.* 2>/dev/null | head -1)"
  fi

  # c) fallback: standard Xauthority files
  if [ -z "$xauth" ] || [ ! -f "$xauth" ]; then
  ...
  # last resort: allow local connections to a local root X server
  if ! xrandr --query >/dev/null 2>&1; then
...
printf "%s\n" "$border"
empty_line
center "BATOCERA CRT SCRIPT — 15 kHz / 25 kHz / 31 kHz for Batocera v44"
empty_line
...
#only for V44.
	cp /userdata/system/Batocera-CRT-Script/GunCon2/gunlight/init-v44.lua /usr/bin/mame/plugins/gunlight/init.lua
#	
