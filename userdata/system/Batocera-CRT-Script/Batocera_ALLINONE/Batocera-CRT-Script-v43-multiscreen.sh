...
if [ "$Drivers_Nvidia_CHOICE" == "Nvidia_Drivers" ]; then
	...
	if [ "$Drivers_Nvidia_CHOICE" == "NOUVEAU" ]&&([ "$Version_of_batocera" == "v39" ]||[ "$Version_of_batocera" == "v40" ]||[ "$Version_of_batocera" == "v41" ]||[ "$Version_of_batocera" == "v42" ]||[ "$Version_of_batocera" == "v43" ]); then
		sed -e 's/.*nvidia-driver=.*/nvidia-driver=false/' -e 's/.*amdgpu=.*/#amdgpu=true/' 	 	/boot/batocera-boot.conf > /boot/batocera-boot.conf.tmp
	else
		if [ "$TYPE_OF_CARD" == "AMD/ATI" ]&&([ "$Version_of_batocera" == "v39" ]||[ "$Version_of_batocera" == "v40" ]||[ "$Version_of_batocera" == "v41" ]||[ "$Version_of_batocera" == "v42" ]||[ "$Version_of_batocera" == "v43" ]); then
...
if ([ "$Version_of_batocera" == "v39" ]||[ "$Version_of_batocera" == "v40" ]||[ "$Version_of_batocera" == "v41" ]||[ "$Version_of_batocera" == "v42" ]||[ "$Version_of_batocera" == "v43" ]); then
...
#######################################################################################
## how to declare second screen
#######################################################################################

video_modeline2="HDMI-1"
display_rotate2=0
RES_SCREEN2="1920x1080"
Rate_screen2="60.00"
H_RES_EDID2=$(( H_RES_EDID ))

sed -i 's/    Option "Ignore" "true"/    #Option "Ignore" "true"/' /etc/X11/xorg.conf.d/10-monitor.conf
...
if [ "$ES_rotation" == "NORMAL" ] || [ "$ES_rotation" == "INVERTED" ]; then	
   Name_First_Script="first_script.sh-generic-multiscreen-v43-NORMAL"
else
   Name_First_Script="first_script.sh-generic-multiscreen-v43-TATE"
fi
...
echo "problem"
	sed -e "s/\[display_mame_rotation\]/$display_mame_rotate/g" -e "s/\[display_fbneo_rotation\]/$display_fbneo_rotate/g" -e "s/\[display_libretro_rotation\]/$display_libretro_rotate/g" \
		-e "s/\[display_standalone_rotation\]/$display_standalone_rotate/g" -e "s/\[display_ES_rotation\]/$display_rotate/g" -e "s/\[es_resolution2\]/$RES_SCREEN2/g" -e "s/\[H_RES2\]/$H_RES_EDID2/g" \
		-e "s/\[card_display\]/$video_modeline/g" -e "s/\[card_display2\]/$video_modeline2/g" -e "s/\[Rate_resolution2\]/$Rate_screen2/g" -e "s/\[es_resolution\]/$RES_EDID/g" /userdata/system/Batocera-CRT-Script/System_configs/First_script/${Name_First_Script} > /userdata/system/scripts/first_script.sh

echo 
else
	sed -e "s/\[display_mame_rotation\]/$display_mame_rotate/g" -e "s/\[display_fbneo_rotation\]/$display_fbneo_rotate/g" -e "s/\[display_libretro_rotation\]/$display_libretro_rotate/g" \
		-e "s/\[display_standalone_rotation\]/$display_standalone_rotate/g" -e "s/\[display_ES_rotation\]/$display_rotate/g" -e "s/\[es_resolution2\]/$RES_SCREEN2/g" -e "s/\[H_RES2\]/$H_RES_EDID2/g" \
		-e "s/\[card_display\]/$video_modeline/g" -e "s/\[card_display2\]/$video_modeline2/g" -e "s/\[Rate_resolution2\]/$Rate_screen2/g" -e "s/\[es_resolution\]/$RES_EDID_SCANNING/g" /userdata/system/Batocera-CRT-Script/System_configs/${Name_First_Script} > /userdata/system/scripts/first_script.sh
fi
...
#################### ZFEBHVUE INVESTIGATION MULTISCREEN ###############################

echo "###################################################"                               >> "$file_BatoceraConf"
echo "#	SECOND SCREEN DECLARATION"                                                       >> "$file_BatoceraConf"
echo "###################################################"                               >> "$file_BatoceraConf"

echo "global.videooutput="$video_modeline                                                >> "$file_BatoceraConf"
echo "global.videooutput2="$video_modeline2                                              >> "$file_BatoceraConf"
echo "display.rotate2="$display_rotate2                                                  >> "$file_BatoceraConf"
echo "es.resolution2="$RES_SCREEN2                                                       >> "$file_BatoceraConf"
